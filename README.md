...
## MockAPI налаштування

1. Створіть файл `.env` в корені проєкту на основі `.env.example`.
2. Додайте ваш base URL:

`REACT_APP_MOCK_API_URL=https://YOUR_PROJECT_ID.mockapi.io`

3. Перезапустіть dev server (`npm start`), якщо він уже був запущений.

Поточні запити:
- `GET /campervans` — каталог камперів
- `POST /contacts` — відправка enquiry з форми бронювання в модалці  
  (якщо ресурсу `contacts` немає в MockAPI, застосунок збереже заявку локально в `localStorage`)
